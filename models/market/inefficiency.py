...
class MarketInefficencyDetector:
    ...
    def detect_inefficiencies(self,
                            prices: pd.DataFrame,
                            volume: pd.Series,
                            sentiment: Optional[pd.Series] = None) -> List[InefficencySignal]:
        ...
        return sorted(
            [s for s in signals if s.confidence >= self.config.get('min_confidence', 0.6)],
            key=lambda x: x.confidence,
            reverse=True
        )
    
    def _detect_pattern_inefficiencies(self, 
                                     prices: pd.DataFrame,
                                     volume: pd.Series) -> List[InefficencySignal]:
        ...
        for pattern in patterns:
            if pattern.type in self.config.get('enabled_patterns', []):
                signals.append(InefficencySignal(
    ...
    def _detect_behavioral_inefficiencies(self,
                                        prices: pd.DataFrame,
                                        volume: pd.Series,
                                        sentiment: Optional[pd.Series]) -> List[InefficencySignal]:
    ...
    def _detect_liquidity_inefficiencies(self,
                                       prices: pd.DataFrame,
                                       volume: pd.Series) -> List[InefficencySignal]:
